...
import ECBAESDecryptor from './aes-ecb-decryptor';
...
class Decrypter {
  ...
  decrypt (method /*=AES-128,AES-128-ECB*/, data, key, iv, callback) {
    if ((method === 'AES-128-ECB') || (this.disableWebCrypto && this.config.enableSoftwareAES)) {
      if (this.logEnabled) {
        logger.log('JS AES decrypt by' + method);
        this.logEnabled = false;
      }
      let decryptor = this.decryptor;
      if (!decryptor || decryptor.aes_method !== method) {
        if (method === 'AES-128') {
          this.decryptor = decryptor = new AESDecryptor();
          this.decryptor.aes_method = method;
        }
        if (method === 'AES-128-ECB') {
          this.decryptor = decryptor = new ECBAESDecryptor();
          this.decryptor.aes_method = method;
        }
      }
      ...
      this.fastAesKey.expandKey()
        .then((aesKey) => {
          ...
          crypto.decrypt(data, aesKey)
            .catch((err) => {
              this.onWebCryptoError(method, err, data, key, iv, callback);
            })
            .then((result) => {
              callback(result);
            });
  ...
  onWebCryptoError (method, err, data, key, iv, callback) {
    if (this.config.enableSoftwareAES) {
      logger.log('WebCrypto Error, disable WebCrypto API');
      this.disableWebCrypto = true;
      this.logEnabled = true;
      this.decrypt(method, data, key, iv, callback);
    } else {
